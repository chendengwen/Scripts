...
testArray.contains(1)
testArray.dropFirst()
dump(testArray)
testArray.elementsEqual(testArray2)
testArray.max()
testArray.min()
testArray.starts(with: [0,1])
//testArray.joined(":", testArray3)
//testArray3.join(":")
...
func removeAnElement<T>( array: [T]) -> GeneratorOf<[T]> {
    var array = array
    var i = 0
    return GeneratorOf {
        if i < array.count {

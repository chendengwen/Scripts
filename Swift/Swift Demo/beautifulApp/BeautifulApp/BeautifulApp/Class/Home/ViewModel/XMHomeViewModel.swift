...
class XMHomeViewModel: NSObject {
    ...
    fileprivate weak var headerView : XMHomeHeaderView!
    fileprivate weak var centerView : UICollectionView!
    fileprivate weak var bottonView : UICollectionView!
    var dataSource : Array<XMHomeDataModel> = Array()
    // 回调
    typealias XMHomeViewModelSuccessCallBack = (_ dataSoure : Array<XMHomeDataModel>) -> Void
    typealias XMHomeVieModelErrorCallBack = (_ error : NSError) -> Void
    var successCallBack : XMHomeViewModelSuccessCallBack?
    var errorCallBack : XMHomeVieModelErrorCallBack?
    override init() {
        super.init()
        
        NotificationCenter.default.addObserver(self, selector: #selector(XMHomeViewModel.sendNotify_success(_:)), name: NSNotification.Name(rawValue: NOTIFY_SETUPHOMEVIEWTYPE), object: nil)
    }
    
    deinit {
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: NOTIFY_SETUPHOMEVIEWTYPE), object: nil)
    }
    ...
    func getData(_ page : Int, successCallBack : XMHomeViewModelSuccessCallBack?, errorCallBack : XMHomeVieModelErrorCallBack?) {
    ...
    // 接受到切换界面的通知
    func sendNotify_success (_ notify : Notification) {
        // 拿到type

...
class XMFindAppDetailController: UIViewController,UIScrollViewDelegate, XMFindAppDetailToolViewDelegate {
    // 模型
    fileprivate var model : XMFindAppModel!
    ...
    fileprivate func getCommentData() {
        ...
        XMNetworkTool.get(APIConfig.API_FindApp_comments, params: params as AnyObject?, success: { [unowned self](json) -> Void in
    ...
    fileprivate lazy var centerView : XMFindAppDetailCenterView = {
    ...
    fileprivate lazy var toolBarView : XMFindAppDetailToolView = {
    ...
    fileprivate lazy var returnBtn : UIButton = {
        ...
        returnBtn.addTarget(self, action: #selector(XMFindAppDetailController.returnBtnDidClick), for: .touchUpInside)
        returnBtn.setImage(UIImage(named: "detail_icon_back_normal"), for: UIControlState())

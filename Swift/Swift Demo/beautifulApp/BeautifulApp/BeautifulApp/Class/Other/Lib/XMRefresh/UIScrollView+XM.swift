...
extension UIScrollView {
    ...
    下拉刷新 第一个参数是方向
    */
    func headerViewPullToRefresh(_ direction : XMRefreashDirection, callback:(() -> Void)?) {
        // 创建headerview
    ...
    */
    ...
    上拉加载更多
    */
    func footerViewPullToRefresh(_ direction : XMRefreashDirection, callback : (()->Void)?) {
        var footView : XMRefreshFooterView!
        if direction == .xmRefreshDirectionHorizontal {
            footView = XMRefreshFooterView(frame: CGRect(x: 0, y: 0, width: XMRefreshViewHeight, height: SCREEN_HEIGHT))
        } else {
            footView = XMRefreshFooterView(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH, height: XMRefreshViewHeight))
        }

...
class ShareSDKUtil: NSObject {
    ...
    class func shareWithModel(_ model : XMShareModel) {
        let publicContent : ISSContent = ShareSDK.content(model.appContent!, defaultContent: "默认分享内容，没内容时显示", image: ShareSDK.image(withUrl: model.imgUrl!), title: model.appTitle!, url: model.appURL!, description: model.appDescription!, mediaType: SSPublishContentMediaTypeNews)
